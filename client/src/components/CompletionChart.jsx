    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="card p-5">
                <h3 className="font-semibold text-sm text-white mb-3">By Priority Tier</h3>
                {tierData.length > 0 ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={tierData} cx="50%" cy="50%" innerRadius={40} outerRadius={65}
                                    ...
                                    {tierData.map((entry) => (
                                        <Cell key={entry.name} fill={TIER_COLORS[entry.name] || '#627890'} />
                                    ))}
                                </Pie>
                    ...
                    <p className="text-xs text-[#64748b] text-center py-10">No data yet</p>
                )}
            </div>

            {/* By status */}
            <div className="card p-5">
                <h3 className="font-semibold text-sm text-white mb-3">By Status</h3>
                {statusData.length > 0 ? (
                    <div style={{ width: '100%', height: 160 }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={statusData} cx="50%" cy="50%" innerRadius={40} outerRadius={65}
                                    ...
                                    {statusData.map((entry) => (
                                        <Cell key={entry.name} fill={STATUS_COLORS[entry.name] || '#627890'} />
                                    ))}
                                </Pie>
                                <Tooltip content={<CustomTooltip />} />
                                <Legend
